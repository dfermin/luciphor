...
void FLRClass::fart() {
	deque<flrStruct>::iterator real_iter;
	deque<double>::iterator globalFDR_iter, localFDR_iter;
	deque<double>::iterator raw_gFDR_iter, raw_lFDR_iter;

	real_iter = realDeq.begin();
	globalFDR_iter = globalFDR.begin();
	localFDR_iter = localFDR.begin();
	raw_gFDR_iter = raw_globalFDR.begin();
	raw_lFDR_iter = raw_localFDR.begin();
	...
	while(real_iter != realDeq.end()) {
		cout << real_iter->specId << "\t"
			 << real_iter->deltaScore << "\t"
			 << *globalFDR_iter << "\t"
			 << *raw_gFDR_iter << "\t"
			 << *localFDR_iter << "\t"
			 << *raw_lFDR_iter << "\n";
